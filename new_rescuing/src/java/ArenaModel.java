...
public class ArenaModel extends GridWorldModel {
	 ...
	 */
	 public void addEmulator(Emulator e){
		 emulator = e;
	 }
	...
	public void checkAndRescue(Location loc) {
		...
		if (hasObject(VIC_POS, loc)) {
			int detected = emulator.detectVictim(true);
			
			remove(VIC_POS, loc);
			
			if(detected == CLEAN)
				System.out.println("No victim present.");
			
			else if(detected == VIC_CRI){
				System.out.println("Patient severity: critical");
				add(VIC_CRI, loc);
				
			} else if(detected == VIC_SER){
				System.out.println("Patient severity: serious");
				add(VIC_SER, loc);
				
			} else {
				System.out.println("Patient severity: minor");
				add(VIC_MIN, loc);
			}
