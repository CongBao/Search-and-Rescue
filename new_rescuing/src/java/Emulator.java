...
public class Emulator {
	...
	// The number of possible victims left, and the number of actual victims left
	private int possibleVictimCount = 5, actualVictimCount = 3;
	 ...
	 *
	 * @param whether the robot knows its location or not (used for random number generation)
	 * @return the id of victim
	 */
	public int detectVictim(boolean knowsLocation) {
		int x = pos[0];
		int y = pos[1];
		
		if(possibleVictimCount < 1)
			System.out.println("Possible victim count = 0");
		else if(actualVictimCount < 1)
			System.out.println("Actual victim count = 0");
		else if(!model.hasObject(ArenaModel.VIC_POS, x, y))
			System.out.println("Arena has no patient here.");
		
		if(possibleVictimCount < 1 || actualVictimCount < 1 || !model.hasObject(ArenaModel.VIC_POS, x, y))
			return ArenaModel.CLEAN;
		
		System.out.println("Generating...");
		
		// Randomly generate whether this tile actually has a victim or not, and then the severity
		int rand = ThreadLocalRandom.current().nextInt(1, possibleVictimCount + 1);
		...
		if(rand <= 3){
			...
			// Generate another number to represent the severity of the patient
